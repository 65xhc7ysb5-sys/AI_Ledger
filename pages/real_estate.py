...
from core.finance import calculate_fv as _fv, calculate_asset_fv as _afv, calculate_max_loan, opportunity_cost as _opp_cost, simulate_scenario_a, simulate_scenario_b, calc_education_opportunity_cost
...
with tab7:
    st.markdown("## 🔀 상급지 매수 vs 의정부 실거주 시나리오 비교")

    # ── 입력 컬럼 ─────────────────────────────────────────────
    col_a, col_b, col_c = st.columns(3)
    with col_a:
        st.markdown("**📅 시뮬레이션 설정**")
        target_year = st.slider("목표 연도", 2027, 2035, 2030, key="sc_target_year")
        sim_years_sc = target_year - 2026
        up_growth = st.slider("상급지 연 가격 상승률 (%)", 1.0, 8.0, 4.0, 0.5,
                               key="sc_up_growth") / 100
        uj_growth = st.slider("의정부 연 가격 상승률 (%)", 1.0, 8.0, 2.0, 0.5,
                               key="sc_uj_growth") / 100
    with col_b:
        st.markdown("**💰 시나리오 B 투자 설정**")
        monthly_saving_sc = st.number_input(
            "월 절약 주거비 (원)", min_value=0, max_value=2_000_000,
            value=0, step=50_000, key="sc_monthly_saving"
        )
        invest_rate_sc = st.slider("절약분 투자 수익률 (%)", 1.0, 8.0, 3.0, 0.1,
                                    key="sc_invest_rate") / 100
    with col_c:
        st.markdown("**🏠 단지 기준가**")
        sc_up_price = st.number_input(
            "상급지 현재가 (억)", min_value=5.0, max_value=15.0,
            value=8.25, step=0.05, key="sc_up_price"
        ) * 1e8
        sc_uj_price = st.number_input(
            "의정부 현재가 (억)", min_value=2.0, max_value=8.0,
            value=4.5, step=0.05, key="sc_uj_price"
        ) * 1e8
        sc_equity_input = float(sim_equity)  # 사이드바 자기자본 재사용
        st.caption(f"자기자본: {sc_equity_input/1e8:.2f}억 (사이드바 연동)")

    # ── 교육비 기회비용 expander ──────────────────────────────
    edu_cost_total = 0.0
    with st.expander("🎓 의정부 통근 기회비용 포함", expanded=False):
        include_edu = st.checkbox("기회비용을 시나리오 B에서 차감", key="sc_include_edu")
        if include_edu:
            e1, e2 = st.columns(2)
            with e1:
                hourly_wage = st.number_input("시급 (원)", min_value=10_000,
                                               max_value=200_000, value=30_000,
                                               step=1_000, key="sc_hourly_wage")
                commute_hrs = st.slider("일 추가 통근시간 (시간)", 0.5, 4.0, 2.0, 0.5,
                                        key="sc_commute_hrs")
            with e2:
                commute_days = st.slider("월 통근일수", 15, 25, 20, key="sc_commute_days")
                transport_cost = st.number_input("월 추가 교통비 (원)", min_value=0,
                                                  max_value=500_000, value=150_000,
                                                  step=10_000, key="sc_transport")
            edu_cost_total = calc_education_opportunity_cost(
                hourly_wage, commute_hrs, commute_days,
                transport_cost, sim_years_sc * 12
            )
            st.info(f"📊 {sim_years_sc}년 누적 기회비용: **{edu_cost_total:,.0f}원 ({edu_cost_total/1e8:.2f}억)**")

    # ── 계산 ─────────────────────────────────────────────────
    sc_a = simulate_scenario_a(
        purchase_price=sc_up_price,
        equity=sc_equity_input,
        rate_annual=sim_rate,         # 사이드바 금리 재사용
        loan_years=sim_years,         # 사이드바 대출기간 재사용
        price_growth_rate=up_growth,
        sim_years=sim_years_sc,
    )
    sc_b = simulate_scenario_b(
        uijeongbu_price=sc_uj_price,
        uijeongbu_growth=uj_growth,
        monthly_rent_saving=monthly_saving_sc,
        invest_rate=invest_rate_sc,
        sim_years=sim_years_sc,
    )

    years   = [d["year"] for d in sc_a]
    a_net   = [d["net_asset"] / 1e8 for d in sc_a]
    b_net   = [(d["net_asset"] - edu_cost_total) / 1e8 for d in sc_b]  # 기회비용 차감

    # ── 손익분기점 ────────────────────────────────────────────
    breakeven_year = None
    for i, (a, b) in enumerate(zip(a_net, b_net)):
        if a > b:
    ...
    fig_sc.add_trace(go.Scatter(
        x=years, y=a_net, mode="lines+markers",
        name="시나리오 A (상급지 매수)",
        line=dict(color="#E63946", width=2.5),
        marker=dict(size=7),
    ))
    fig_sc.add_trace(go.Scatter(
        x=years, y=b_net, mode="lines+markers",
        name="시나리오 B (의정부 실거주+투자)",
    ...
    # 2029년 수직선
    fig_sc.add_vline(
        x=2029, line_dash="dash", line_color="gray", line_width=1.5,
        annotation_text="목표 시점 2029", annotation_position="top right",
    )

    # 교차점 annotation
    if breakeven_year:
        be_idx = years.index(breakeven_year)
        fig_sc.add_annotation(
            x=breakeven_year, y=a_net[be_idx],
            text=f"A/B 역전 시점<br>{breakeven_year}년",
            showarrow=True, arrowhead=2,
            bgcolor="#E63946", font=dict(color="white", size=11),
            arrowcolor="#E63946",
        )

    fig_sc.update_layout(
        title="📈 시나리오별 순자산 추이",
        xaxis_title="연도", yaxis_title="순자산 (억원)",
        hovermode="x unified",
    ...
    final_a = a_net[-1] if a_net else 0
    final_b = b_net[-1] if b_net else 0
    mc1.metric(f"A 순자산 ({target_year}년)", f"{final_a:.2f}억")
    mc2.metric(f"B 순자산 ({target_year}년)", f"{final_b:.2f}억",
               delta=f"{'A 우위' if final_a > final_b else 'B 우위'} {abs(final_a-final_b):.2f}억",
               delta_color="inverse" if final_a > final_b else "normal")
    if breakeven_year:
        mc3.metric("A > B 역전 시점", f"{breakeven_year}년",
                   delta=f"{breakeven_year - 2026}년 후")
    else:
        mc3.metric("A > B 역전 시점", "분석 기간 내 없음", delta="B가 계속 우위")

    # ── 요약 테이블 ───────────────────────────────────────────
    st.markdown("#### 📋 연도별 순자산 비교")
    summary_df = pd.DataFrame({
        "연도":          years,
        "A 순자산 (억)": [round(v, 2) for v in a_net],
        "B 순자산 (억)": [round(v, 2) for v in b_net],
        "차이 A-B (억)": [round(a - b, 2) for a, b in zip(a_net, b_net)],
    })
    st.dataframe(
        summary_df,
        use_container_width=True,
        hide_index=True,
        column_config={
            "A 순자산 (억)": st.column_config.NumberColumn(format="%.2f억"),
            "B 순자산 (억)": st.column_config.NumberColumn(format="%.2f억"),
            "차이 A-B (억)": st.column_config.NumberColumn(format="%.2f억"),
        },
    )
